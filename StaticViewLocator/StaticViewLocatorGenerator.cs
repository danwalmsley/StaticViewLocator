...
public sealed partial class StaticViewLocatorGenerator : IIncrementalGenerator
{
    ...
    private readonly struct ResolvedViewMapping
    {
        public ResolvedViewMapping(INamedTypeSymbol viewModelType, INamedTypeSymbol viewType)
        {
    ...
    private const string AttributeText =
        ...
        public enum ViewResolutionMode
        {
            Exact,
            ExactThenBaseTypes,
            ExactThenInterfaces,
        ...
        public sealed class StaticViewLocatorAttribute : Attribute
        {
            ...
            public ViewResolutionMode GeneratedAdapterResolutionMode { get; set; } =
                ViewResolutionMode.ExactThenBaseTypesThenInterfaces;
        ...
        List<Diagnostic> diagnostics)
    {
        ...
        source.AppendLine();
        var resolvedMappings = new List<ResolvedViewMapping>();
        var resolvedViewModels = new HashSet<INamedTypeSymbol>(SymbolEqualityComparer.Default);
        ...
        foreach (var viewModelSymbol in relevantViewModels)
        {
            ...
            var viewSymbol = inferredMappings.TryGetValue(viewModelSymbol, out var inferredViewSymbol)
                ? inferredViewSymbol
                : compilation.GetTypeByMetadataName(metadataNameView);
            var isSupportedView = viewSymbol is not null && IsSupportedView(viewSymbol, viewBaseTypes);

            if (viewSymbol is null || !isSupportedView || viewSymbol.IsGenericType)
            {
                continue;
            }
            ...
            resolvedMappings.Add(new ResolvedViewMapping(viewModelSymbol, viewSymbol));
            resolvedViewModels.Add(viewModelSymbol);
        }

        if (generateIViewLocator || generateIDataTemplate)
        {
            AddCompileTimeFallbackMappings(
                locatorSymbol,
                relevantViewModels,
                resolvedMappings,
                resolvedViewModels,
        ...
        foreach (var mapping in resolvedMappings.OrderBy(
                     static mapping => mapping.ViewModelType.ToDisplayString(),
                     StringComparer.Ordinal))
        {
            source.AppendLine(
                $"\t\t[typeof({GetSourceTypeReference(mapping.ViewModelType)})] = () => new {GetSourceTypeReference(mapping.ViewType)}(),");
        }
        ...
        if (generateIViewLocator || generateIDataTemplate)
        {
            AppendGeneratedViewResolver(source, locatorSymbol, resolvedMappings);
        }
